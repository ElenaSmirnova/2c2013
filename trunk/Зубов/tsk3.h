...
public:
    ...
    void Print(){
        for (int i = 0;i < len;i++) cout<<((int)v[i]+26000)%26<<" ";
        cout<<"\n";
    }
...
public:
    ...
    Matrix Inverse(double det) {
        ...
        while ((a*x)%26 != 1) a++;
        for (int i = 0;i < d;i++) {
            for (int j = 0;j < d;j++) Alg.A[i].v[j] *= a;
        }
        return(Alg);
    }
...
int main() {
    ...
    cout<<"Write string length of d\n";
    string s;
    cin>>s;
    cout<<"Write the cipher length of d\n";
    for (int i = 0;i < u;i++) cin>>zero.v[i];
    Vector S;
    for (int i = 0;i < d;i++) S.v[i] = (int)s.at(i) - (int)'A';
    cout<<"The encoded string is\n";
    M.Multiply(M,S).Print();
    cout<<"The decoded string is\n";
    Vector T;
    T = M.Multiply(M.Inverse(M.Det()),zero);
    for (int i = 0;i < d;i++) cout<<(char)(((int)T.v[i]+26000000)%26+'A');
    return(0);
}